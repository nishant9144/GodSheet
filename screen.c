...
#include <termios.h>
#include <unistd.h>
#include "backend.h"  // NEW: Include backend header
...
// NEW: Frontend state (separate from backend data)
typedef struct {
    Spreadsheet *backend_sheet;  // Backend spreadsheet
    int currentTopRow;
    int currentLeftCol;
    int mode;
    int cursorRow;
...
// NEW: Use backend's cell reference parser
int parseCellRef(const char *ref, int *row, int *col) {
    CellRef cellref;
    if (!parse_cell_ref(ref, &cellref)) return 0;
    *row = cellref.row;
    *col = cellref.col;
    return 1;
}
...
// NEW: Updated to use backend functionsf
void processCellCommand(FrontendState *frontend, char *command) {
    char *equals = strchr(command, '=');
    if (!equals) return;
    ...
    *equals = '\0';
    ...
    // Remove spaces
    while (*cellRef == ' ') cellRef++;
    while (*value == ' ') value++;

    int row, col;
    if (!parseCellRef(cellRef, &row, &col) || 
        row >= frontend->backend_sheet->rows || 
        col >= frontend->backend_sheet->cols) {
        printf("Invalid cell reference\n");
        return;
    }

    if (value[0] == '=') {
        // Set formula using backend
        CalcError err = sheet_set_formula(frontend->backend_sheet, row, col, value + 1);
        if (err != OK) printf("Formula error: %d\n", err);
    } else {
        // Set value using backend
        CalcError err = sheet_set_value(frontend->backend_sheet, row, col, atoi(value));
        if (err != OK) printf("Value error: %d\n", err);
    }
}

// NEW: Initialize frontend state with backend
FrontendState *initFrontend(int rows, int cols) {
    FrontendState *frontend = (FrontendState *)malloc(sizeof(FrontendState));
    frontend->backend_sheet = sheet_create(rows, cols);
    frontend->currentTopRow = 0;
    frontend->currentLeftCol = 0;
    frontend->mode = VIEW_MODE;
    frontend->cursorRow = 0;
    frontend->cursorCol = 0;
    frontend->viewportRows = SHEET_HEIGHT;
    frontend->viewportCols = SHEET_WIDTH;
    return frontend;
}

// NEW: Display backend cell data
void clearScreen() {
    printf("\x1b[2J\x1b[H");
}

void setColor(int highlight) {
    if (highlight) printf("\x1b[47m\x1b[30m");
    else printf("\x1b[0m");
}
...
void displaySheet(FrontendState *frontend) {
    clearScreen();
    int endRow = frontend->currentTopRow + frontend->viewportRows;
    int endCol = frontend->currentLeftCol + frontend->viewportCols;

    if (endRow > frontend->backend_sheet->rows) endRow = frontend->backend_sheet->rows;
    if (endCol > frontend->backend_sheet->cols) endCol = frontend->backend_sheet->cols;

    // Display mode and navigation info
    printf("Mode: %s    Page: Row %d-%d, Col %d-%d\n",
           frontend->mode == VIEW_MODE ? "VIEW" : "EDIT",
           frontend->currentTopRow + 1, endRow,
           frontend->currentLeftCol + 1, endCol);

    // Display column headers
    printf("      ");
    for (int j = frontend->currentLeftCol; j < endCol; j++) {
        char colRef[4];
        numToColRef(j, colRef);
        printf("%-*s", CELL_WIDTH, colRef);
    }
    ...
    // Display cells
    for (int i = frontend->currentTopRow; i < endRow; i++) {
        printf("%4d |", i + 1);
        for (int j = frontend->currentLeftCol; j < endCol; j++) {
            Cell *cell = &frontend->backend_sheet->cells[i][j];
            printf(" ");
            if (i == frontend->cursorRow && j == frontend->cursorCol) {
                setColor(1);
                printf("%-*d", CELL_WIDTH - 1, cell->value);
                setColor(0);
            } else {
                printf("%-*d", CELL_WIDTH - 1, cell->value);
            }
...
// NEW: Updated to use FrontendState
void handleNavigation(FrontendState *frontend, char key) {
    switch (key) {
        case ARROW_UP:
            if (frontend->cursorRow > 0) frontend->cursorRow--;
            break;
        case ARROW_DOWN:
            if (frontend->cursorRow < frontend->backend_sheet->rows - 1) frontend->cursorRow++;
            break;
        case ARROW_LEFT:
            if (frontend->cursorCol > 0) frontend->cursorCol--;
            break;
        case ARROW_RIGHT:
            if (frontend->cursorCol < frontend->backend_sheet->cols - 1) frontend->cursorCol++;
            break;
    }
}
...
int main(int argc, char *argv[]) {
    ...
    FrontendState *frontend = initFrontend(rows, cols);
    configureTerminal();

    char input;
    while (1) {
        displaySheet(frontend);
        printf("> ");
        input = getchar();

        if (input == 'q') break;
        else if (input == 'm') frontend->mode = !frontend->mode;
        else handleNavigation(frontend, input);
    }

    restoreTerminal();
    sheet_destroy(frontend->backend_sheet);
    free(frontend);
    return 0;
}